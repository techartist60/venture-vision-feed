...
export function exportWebScanToPdf(data: WebScanData) {
  ...
  const dateText = data.scanDate 
    ? `Scanned on ${new Date(data.scanDate).toLocaleDateString('en-US', { 
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text(data.websiteTitle, 14, yPosition);
  yPosition += 6;
  ...
  yPosition += 5;
  doc.setFont('helvetica', 'normal');
  const mainConceptLines = doc.splitTextToSize(data.analysis.mainConcept, pageWidth - 28);
  ...
  yPosition += 5;
  doc.setFont('helvetica', 'normal');
  const problemLines = doc.splitTextToSize(data.analysis.problem, pageWidth - 28);
  ...
  yPosition += 5;
  doc.setFont('helvetica', 'normal');
  const audienceLines = doc.splitTextToSize(data.analysis.targetAudience, pageWidth - 28);
  ...
  yPosition += 5;
  doc.setFont('helvetica', 'normal');
  data.analysis.coreFeatures.forEach(feature => {
  ...
  yPosition += 5;
  doc.setFont('helvetica', 'normal');
  doc.text(data.analysis.keywords.join(', '), 14, yPosition);
  yPosition += 15;
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(`Similar Websites Found (${data.similarWebsites.length})`, 14, yPosition);
  yPosition += 10;

  if (data.similarWebsites.length === 0) {
    ...
    const tableData = data.similarWebsites.map(website => [
      website.name,
      website.url,
